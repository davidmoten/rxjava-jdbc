...
final class QuerySelectOperation {
	...
	private static final Logger log = LoggerFactory
			.getLogger(QuerySelectOperation.class);
	 ...
	 * @return
	 */
	static <T> Observable<T> execute(QuerySelect query,
			List<Parameter> parameters, Func1<? super ResultSet, T> function) {
		return Observable.create(new QuerySelectOnSubscribe<T>(query,
				parameters, function));
	}
	...
	private static class QuerySelectOnSubscribe<T> implements OnSubscribe<T> {
		...
		private void executeQuery(Subscriber<? super T> subscriber)
			...
			if (keepGoing) {
				try {
					...
					throw new SQLException("failed to run sql=" + query.sql(),
							e);
				}
			}
		}
		 ...
		 * @throws SQLException
		 */
		private void processRow(Subscriber<? super T> subscriber)
				throws SQLException {
			checkSubscription(subscriber);
			if (!keepGoing)
				return;
