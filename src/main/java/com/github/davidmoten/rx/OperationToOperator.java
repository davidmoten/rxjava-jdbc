...
public final class OperationToOperator<R, T> implements Operator<R, T> {
	...
	private static class SingleSubscribeSubject<T> extends Observable<T>
			implements Observer<T> {

		private final SingleSubscribeOnSubscribe<T> subscriberHolder;

		protected SingleSubscribeSubject(
				SingleSubscribeOnSubscribe<T> onSubscribe) {
			super(onSubscribe);
			...
			public void call(Subscriber<? super T> subscriber) {
				if (this.subscriber != null)
					throw new RuntimeException(
							"only once subscription is allowed");
